      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] gap-0 xl:grid-cols-[280px_1fr]">
        <aside className="border-b border-white/10 bg-[#08101a] px-5 py-6 xl:border-b-0 xl:border-r">
          <div className="sticky top-0">
            <Link href="/admin" className="inline-flex items-center rounded-full border border-cyan-400/30 bg-cyan-400/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.34em] text-cyan-200">
              Modaily Admin
            </Link>
            <h1 className="mt-6 font-display text-4xl text-white">Control room</h1>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Storefront’dan alohida admin panel. Hozir authsiz ishlaydi va `User`, `Product`, `Category`
            ...
            <div className="mt-8 space-y-2">
              {adminLinks.map((link) => (
                <Link
                  key={link.key}
                  href={link.key === "overview" ? link.href : `/admin${link.href}`}
            ...
            <div className="mt-8 rounded-[1.75rem] border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <span className="text-sm text-slate-200">Admin UI and CRUD API are active</span>
              </div>
            </div>
          </div>
        </aside>
