...
export function SiteFooter({ locale, dictionary, siteSettings }: SiteFooterProps) {
  ...
  return (
    <footer
      ...
      style={{
        backgroundImage:
          "radial-gradient(circle at 18% 82%, rgba(255,66,104,0.18) 0, transparent 28%), radial-gradient(circle at 52% 18%, rgba(220,22,63,0.18) 0, transparent 34%), radial-gradient(circle at 84% 54%, rgba(255,80,120,0.12) 0, transparent 26%), linear-gradient(135deg, #7e0d22 0%, #8c1024 18%, #b10f2b 52%, #9b1127 78%, #7a0d20 100%)"
      }}
    >
      <div className="pointer-events-none absolute inset-0 opacity-20 [background-image:linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] [background-size:140px_140px]" />
      <div className="relative mx-auto max-w-[1180px] px-4 py-12 lg:px-6 lg:py-16">
        <div className="border-t border-white/30 pt-10">
          <div className="grid gap-10 lg:grid-cols-[1.2fr_0.55fr_0.9fr]">
            <div>
              <p className="text-[2.45rem] font-black uppercase tracking-[-0.06em] text-white">{siteSettings.brandName}</p>
            ...
            <div>
              ...
              <p className="mt-5 max-w-[270px] text-[14px] leading-6 text-white/86">{siteSettings.footerAddress}</p>
              <p className="mt-8 text-[14px] font-medium text-white">{labels.subscribe}</p>
          ...
          <p className="mt-12 text-center text-[11px] text-white/48">(c) 2026 {siteSettings.brandName}. All rights reserved.</p>
